...
client.connect();
var checkRememberMe = false;
app.get('/users/login', checkAuthenticatedSuccessfull, (req, res) => {
    count=0;
    res.render('login');
});

app.get('/users/register', checkAuthenticatedSuccessfull, (req, res) => {
    res.render('register');
});
...
app.post('/users/register', async (req, res) => {
    ...
    client.query(`select* from users where username='${username}' or email='${email}';`, (err, results) => {
        if (err) {
            throw err;
        }
        if (results.rows.length > 0) {
            res.render('register', {
                message: 'Đăng ký thất bại do tên đăng nhập hoặc địa chỉ email này đã được đăng ký từ trước đó!'
            });
        }
        else {
            client.query(`insert into users(username,password,email) values('${username}','${hashedPassword}','${email}');`, (err, results) => {
                if (err) {
                    throw err;
                }
                res.render('register', {
...
app.post('/users/dashboard',checkAuthenticated, upload.single('add_link_img_product') , (req, res) => {
...
app.get('/users/dashboard', checkAuthenticated, (req, res) => {
    var check = req.session.passport.user.split(' ');
    if (check[1] == 'true') {
        // client.query(`select*
        // from public."Products" as p, public."Categories" as c
        // where p."CatID"=c."CatID"
        ...
        //     var msg = '';
        //     for (var i = 0; i < 30; i++) {
        //         msg = msg + `<div class="card-item"><div class="card-item-info"><h3>Tên sản phẩm</h3><ul><li>${results.rows[i].ProName}</ul><h3>Mô tả ngắn gọn:</h3><ul><li>${results.rows[i].TinyDes}</ul><h3>Thông tin chi tiết:</h3>${results.rows[i].FullDes}<div class="price"><h4>Giá tiền:</h4><ul><li>${results.rows[i].Price}₫</ul></div><div class="type"><h4>Loại hàng:</h4><ul><li>${results.rows[i].CatName}<ul></div><div class="quantity"><h4>Số lượng:</h4><ul><li>${results.rows[i].Quantity}<ul></div></div><div class="card-item-img"><img src="http://localhost:3000/main.jpg" alt="cac"></div></div>`;
        //     }
        ...
        //});
            ...
            client.query(`select a."ProName" from public."Products" as a order by a."ProID"`, (err,results) => {
                ...
                req.flash("list_option_product1",`${msg1}`);
                if (count==1){
                    saveTime=req.session.cookie.expires.getTime();
                }
                req.flash("hichic",`<p id="demo1">${saveTime}</p>`);
                res.render('productsview');
            })
    }
});
app.get('/viewsbookproduct',(req, res, next) => {
    ...
    client.query(`select count(*) from public."Products" as a, public."Categories" as b where a."CatID"=b."CatID" and a."CatID"=1`, (err,results) => {
        ...
        client.query(`select* from public."Products" as a, public."Categories" as b where a."CatID"=b."CatID" and a."CatID"=1`, (err, results) => {
            ...
            var msg=''
            for(var i=0;i<count_book_products;i++){
                msg = msg + `<div class="card-item"><div class="card-item-img"><img src="${results.rows[i].LinkImg}" alt=""></div><div class="card-item-info"><h3>Tên sản phẩm</h3><ul><li>${results.rows[i].ProName}</ul><h3>Mô tả ngắn gọn:</h3><ul><li>${results.rows[i].TinyDes}</ul><h3>Thông tin chi tiết:</h3>${results.rows[i].FullDes}<div class="price"><h4>Giá tiền:</h4><ul><li>${results.rows[i].Price}₫</ul></div><div class="type"><h4>Loại hàng:</h4><ul><li>${results.rows[i].CatName}<ul></div><div class="quantity"><h4>Số lượng:</h4><ul><li>${results.rows[i].Quantity}<ul></div></div></div>`;
            }
            client.query(`select a."ProName" from public."Products" as a order by a."ProID"`, (err,results) => {
                if (err) throw err;
                var msg1='';
                for(var i=0;i<results.rows.length;i++) msg1=msg1+`<option value="${i+1}">${results.rows[i].ProName}<option>`;
                res.render('productsview',{
                    list_option_product: msg1,
                    data_dashboard: msg
                })
            })
        })
...
app.get('/viewsphoneproduct',(req, res, next) => {
    if (req.isAuthenticated()) {
...
app.get('/viewscameraproduct',(req, res, next) => {
    if (req.isAuthenticated()) {
...
app.get('/viewsshirtshoeproduct',(req, res, next) => {
    if (req.isAuthenticated()) {
...
app.get('/viewslaptoppcproduct',(req, res, next) => {
    if (req.isAuthenticated()) {
...
app.get('/viewsdtsproduct',(req, res, next) => {
    if (req.isAuthenticated()) {
...
app.get('/viewsanotherproduct',(req, res, next) => {
    if (req.isAuthenticated()) {
...
app.get('/viewsProduct', (req, res, next) => {
    if (req.isAuthenticated()) {
...
passport.use(new LocalStrategy({
    ...
    client.query(`select* from users where username='${username}'`, (err, results) => {
        if (err) throw err;
        if (results.rows.length > 0) {
            ...
            bcrypt.compare(password, user.password, (err, isMatch) => {
                ...
                if (isMatch) {
                    return done(null, {
                        ...
                        email: `${user.email}`,
                        bu: `${user.username} ${checkRememberMe}`
                    });
                }
                else {
            ...
            return done(null, false, { message: "Tài khoản này chưa được đăng ký" });
        }
    })
}));
...
passport.deserializeUser((bu, done) => {
    var buu = bu.split(' ');
    client.query(`select* from users where username='${buu[0]}'`, (err, results) => {
        if (err) {
            throw err
        }
...
function checkAuthenticated(req, res, next) {
    ...
    req.flash("notification",`<div class="notification" onload="handle_onload_notification()">time_out</div>`)
    res.redirect('/users/login');
}
function checkAuthenticatedSuccessfull(req, res, next) {
    if (req.isAuthenticated()) {
